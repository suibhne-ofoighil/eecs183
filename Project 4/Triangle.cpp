...
void Triangle::read(istream& ins) {
    ...
    //if reads in this way, it is the first form
    if (ins >> v1 >> v1color){
        ins >> v2 >> v2color >> v3 >> v3color;
        vertexOneColor = v1color;
        vertexTwoColor = v2color;
        vertexThreeColor = v3color;
    }
    //if >> operator failed, it means it is the second form
    if (ins.fail()) {
        ins.clear();
        ins >> v2 >> v3 >> color;
        vertexOneColor = color;
