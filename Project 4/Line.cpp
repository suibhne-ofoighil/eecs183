 ...
 * Uniquename: suibhne
 *
 * Line class, one of the shapes which can be drawn.
 */
...
Line::Line(Point start_in, Point end_in, Color color_in) {
    start = start_in;
    end = end_in;
    lineColor = color_in;
}

void Line::setStart(Point start_in) {
    start = start_in;
}
...
void Line::setColor(Color color_in) {
    lineColor = color_in;
}
...
void Line::read(istream& ins) {
    ...
    if (ins >> startPoint >> endPoint >> color)
    {
        start = startPoint;
        end = endPoint;
        lineColor = color;
